...
My scripts for downloading the refernence genome for great tits (from a bird named Abel) with additional annotations
...
My scripts for downloading genomes for outgroup birds (blue tit and ground tit)
```bash
...
# My scripts for downloading genomes for outgroup birds - BLACK CAPPED CHICKADEE (Poecile atricapillus )
```bash
...
#My scripts for downloading genomes for outgroup birds - PITTA SORDIDA (HOODED PITA)
```bash
...
#My scripts for downloading genomes for outgroup birds - NEOPELMA CHRYSOCEPHALUM (SAFFRON-CRESTED TYRANT_MANAKIN)
```bash
